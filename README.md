...
Lunar Finance operates aggregation infrastructure spanning 40+ DeFi protocol integrations across multiple ecosystems. Stellar is added as a new set of adapters inside that existing system.
...
|---|---|---|
| Soroswap | Soroban-native DEX/aggregator | Router contract via Stellar RPC, as a liquidity source in our aggregation layer |
| Aquarius (AQUA) | Soroban AMM — the deepest AMM/DEX liquidity on Stellar (~$45M TVL, DefiLlama, Aug 2026) | AMM router contract via Stellar RPC |
| SushiSwap | Soroban DEX with v3-style concentrated liquidity, live on Stellar since Feb 2026 | Router contract via Stellar RPC — a new Soroban implementation behind the same adapter interface as the SushiSwap integrations Lunar runs on EVM chains |
...
- **Soroban-based venues (Aquarius, Soroswap, SushiSwap)** are called via Stellar RPC. Router contract IDs are treated as versioned configuration, verified against official documentation at integration time and monitored for changes (e.g., after protocol upgrades or network resets) — never hardcoded permanently.
...
flowchart TD
    ...
    FAN --> SS["Soroswap router<br/>Stellar RPC"]
    FAN --> AQ["Aquarius AMM router<br/>Stellar RPC"]
    FAN --> SU["SushiSwap CL pools<br/>Stellar RPC"]
...
- Stellar RPC access for AMM contract calls and simulation
...
**A note on USDC:** USDC on Stellar is natively issued by Circle, and Circle's CCTP is live on Stellar as of May 2026 — native USDC moves between Stellar and other CCTP chains by burn-and-mint, with no wrapped assets. Stellar support is CCTP V2 (domain 27). Fast versus Standard Transfer is selected at burn time on the source chain: Circle marks Stellar N/A as a Fast Transfer *source* because Stellar's standard attestation is already fast — not because Fast is unsupported — and inbound routes from Fast-enabled source chains settle at Fast speed, so settlement-time estimates shown to users follow the source chain's attestation tier. Inbound transfers route through Circle's `CctpForwarder`, which must be set as both `mintRecipient` and `destinationCaller` — Circle documents that a wrong value in either field strands funds unrecoverably, so both are enforced as validated invariants with dedicated tests — and which adapts address encoding and USDC precision to Stellar: CCTP messages carry 6-decimal amounts against Stellar's 7, and Stellar-originated burns truncate the seventh decimal, which quoting and reconciliation account for explicitly. Given USDC's centrality to Lunar's other product features (fiat ramp, Lunar Spend), the engine defaults to CCTP whenever both source and destination chains support it; first-wave protocols cover USDC routes until the CCTP adapter ships.
...
flowchart TD
    ...
    PREP --> BURN
    READY -- yes --> BURN["Source chain: native USDC burned<br/>CCTP V2 — Fast or Standard per source-chain finality"]
...
- **Adapter pattern, consistent with Section 3**: each protocol is wrapped in an adapter exposing `getCrossChainQuote(sourceChain, destChain, assetIn, assetOut, amount)`, returning estimated output, fees, and estimated settlement time. The destination-side asset is explicit because representation differs by protocol — canonical native USDC delivered via CCTP is not the same deliverable as an ITS-wrapped representation via Axelar — so routes are compared on what the recipient actually receives, using the same full asset identities as Section 3.3.
- **Routing decision logic**: for a given cross-chain request, the engine queries all applicable adapters in parallel, applies an eligibility gate, and only then ranks the survivors by a weighted score of price, fee, and speed/reliability (Section 6). The gate is a hard filter, not a score component: a route delivering a non-canonical representation of an asset is excluded whenever a canonical route exists (a wrapped stand-in never outranks native USDC via CCTP on price alone), and when only a non-canonical route exists it is offered labeled as such rather than silently ranked.
- **Settlement tracking**: each protocol settles through its own sequence of states — source lock/burn, then attestation, solver fill, or message relay, then destination release/mint — so each adapter implements an idempotent per-protocol settlement state machine rather than a generic two-confirmation model. Stellar-side completion is detected primarily via Stellar RPC's `getEvents` (contract-emitted transfer events — the right shape for watching one asset across many recipients), with Horizon streaming (Server-Sent Events, cursor-resumable) as a secondary listener on classic payment paths.
...
flowchart TD
    ...
    MINT --> DETECT2["Stellar-side detection:<br/>Stellar RPC events and Horizon streaming"]
...
- **Signing flow**: transactions are constructed with the Stellar SDK and passed to the connected wallet through the kit's SEP-43 interface — XDR envelope signing (`signTransaction`), plus authorization-entry signing (`signAuthEntry`) where a Soroban invocation requires it — then submitted through Horizon (or Stellar RPC for contract transactions).
- **Contract accounts**: smart wallets (passkey signers) authorize via Soroban auth entries rather than envelope signatures. They are not a first-class launch target, and they are scoped honestly as follow-on work rather than a free extension: the SEP-43 base covers the signing interface, but contract accounts also change the account model — no classic trustlines, different reserve and sequence handling, and a separate submission/fee account — so the trustline and concurrency flows in Sections 7.1 and 7.6 do not transfer as-is. Protocol 27's address-bound Soroban credentials (CAP-71) is the relevant current-cycle primitive here and is tracked as part of that follow-on scope.
- **Account and trustline state**: on connection, the account's balances and trustlines are read via Horizon (`/accounts/{account_id}`); balances held in Soroban contract state, which Horizon's account endpoint does not surface, are read via Stellar RPC.
...
flowchart TD
    ...
    SIGN_SOR --> SUBMIT2["Submit via Stellar RPC"]
...
Extend the routing engine so that every route displayed as executable has passed validation appropriate to its type before submission — for Soroban routes, a recent `simulateTransaction` — catching failures pre-execution rather than on-chain. Discovery is wide but simulation is focused: hosted aggregation APIs handle the broad scan, and finalist routes receive full simulation (Section 6.2).
...
- **Ranking**: eligibility (asset canonicality, security policy — Section 4.3) is applied as a hard filter first; the surviving routes are ranked by a composite of price, fees, estimated settlement time, and simulation-confirmed reliability. The top route is preselected; alternates remain user-selectable.
- **Failure filtering**: routes that fail simulation are discarded before display.
- **Execution**: on confirmation, the engine builds the final transaction (or transaction set for multi-hop/cross-chain routes), hands it to the wallet layer for signing, and submits via Horizon/Stellar RPC.
...
flowchart TD
    ...
    SIGN6 --> SUB6["Submit via Horizon / Stellar RPC"]
...
- Stellar RPC simulation endpoint
...
Non-native Stellar assets require an account trustline before they can be held, and each trustline raises the account's minimum XLM reserve by 0.5 XLM — so a first-time recipient may lack both the trustline and the XLM to open it. For accounts without spendable XLM (the common case for users bridging into Stellar for the first time), sponsored reserves (CAP-33) let Lunar's infrastructure cover the reserve, so onboarding does not dead-end on "acquire XLM first"; inbound cross-chain routes (Section 4) run the same readiness check — account exists, trustline present, reserve covered — before the source leg is initiated. Sponsorship is a bounded budget, not an open-ended liability: per-account and aggregate XLM exposure caps, an asset allowlist, and rate limits gate who receives sponsored trustlines, and CAP-33 sponsorships are revocable, so reserve exposure is monitored and recoverable rather than Sybil-farmable. The flow detects missing trustlines pre-swap and resolves them according to the route type: on classic routes Lunar builds itself, the trustline operation is bundled into the same transaction as the swap; Soroban transactions are single-operation by protocol rule, so there the trustline is established in its own transaction immediately before the swap; and provider-prepared routes (Stellar Broker's channel-account transaction sets) are never modified client-side — altering a prepared envelope would invalidate it — so trustline readiness is ensured before the provider quote is finalized. Wherever possible trustlines are pre-established at connect or quote time so no extra step is visible at swap time. Either way, the user sees a swap, not a raw Stellar-specific error.
...
flowchart TD
    ...
    ROUTE -- "classic path built by Lunar" --> BUNDLE["Single transaction:<br/>trustline operation bundled with the swap<br/>classic transactions allow multiple operations"]
    ...
    ROUTE -- "provider-prepared, e.g. Stellar Broker" --> PROVTL["Trustline established before the provider<br/>quote is finalized — prepared envelopes<br/>are never modified client-side"]
...
- All adapters are built and tested against Stellar **testnet** (testnet Horizon and Stellar RPC) before any mainnet exposure.
...
- SDF's public Horizon and Stellar RPC endpoints cover testnet; **mainnet Stellar RPC is a commercial dependency** — a primary provider plus an independent fallback, budgeted as a metered cost rather than assumed free.
- Horizon's announced long-term deprecation (in favor of Stellar RPC and richer data APIs; no sunset date — Horizon remains protocol-current) is tracked, not feared: account-state and streaming reads sit behind Lunar's data-access layer so they can migrate without an architecture change, Stellar RPC's `sendTransaction` already covers classic and contract transactions alike, and the reconciliation ledger persists settlement events durably rather than depending on Stellar RPC's bounded (~7-day) history window.
...
- Stellar's protocol upgrade cadence is tracked as routine maintenance: Protocol 20 → 27 shipped secp256r1 signers, unified asset events, archived-entry auto-restore, and address-bound Soroban credentials (CAP-71) within three years — mainnet reached Protocol 27 in July 2026 — and adapter/SDK compatibility work is planned for, not exceptional.
